...
static void Response(byte *bufferPointer, int sum)
{
    ...
    *pointer = Byte0(crc);
    sum = sum + 2;
    System.Device.Usart2.Write(bufferPointer, sum);
}
...
static void WriteSingle0x(ushort address)
{
    ...
    System.Device.Usart2.Write(RxdBuffer, RxdCounter);    //应答帧，跟接收帧相同
}
...
static void Read3x(ushort address)
{
    ...
    {
        data = *(ushort *)(DataBase + (uint)Block3x[address + i]);
        *pointer++ = Byte1(data);
        *pointer++ = Byte0(data);
    }
...
static void WriteSingle4x(ushort address)
{
    ...
    *(ushort *)(DataBase + (uint)Block4x[address]) = data;
    ...
    System.Device.Usart2.Write(RxdBuffer, RxdCounter);
}
